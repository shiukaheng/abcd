...
from abcd.core.BasePointCloud import BasePointCloud
from abcd.core.View import ViewWithRes
from abcd.geometry.bounding_box import BoundingBox
from abcd.helpers.math import create_scaled_sigmoid, inverse_sigmoid
from abcd.helpers.path import mkdir_p
from abcd.helpers.spherical_harmonics import rgb_to_sh
from abcd.helpers.transforms import build_covariance_from_scaling_rotation
...
class GaussianModel(nn.Module):
    ...
    def get_scales_activations(scales_range: Union[Tuple[float, float], None] = None):
        ...
        if scales_range is not None:
            scaled_sigmoid, inverse_scaled_sigmoid = create_scaled_sigmoid(
                scales_range[0], scales_range[1]
            )
    ...
    def __init__(
        ...
        background_color: torch.Tensor = torch.tensor(
        ...
        ] = None,  # Minimum and maximum scale for sigmoid
    ):
        super().__init__()
        ...
        self.scales_activation, self.scales_inverse_activation = (
            self.get_scales_activations(scales_range)
        )
    ...
    def forward(
        self, camera: ViewWithRes, active_sh_degree: Union[int, None] = None
    ...
    def backprop_stats(self):
        ...
        if self.viewspace_points.grad is not None:
            ...
            self._gradient_accumulator_denominator[visible_gaussians] += (
                1  # Add 1 to denominator for each point, so we can average the gradient later
            )

        self.max_radii2D[visible_gaussians] = torch.max(
            self.max_radii2D[visible_gaussians],
    ...
    def mean_gradient_magnitude(self):
        ...
        if denom_nan > 0:
            print(
                f"Warning: _gradient_accumulator_denominator contains {denom_nan} NaN values."
            )
        m[torch.isnan(m)] = 0
        return m
    ...
    def from_point_cloud(
        ...
        with torch.no_grad():
            ...
            scales_activation, inverse_scale_transform = (
                GaussianModel.get_scales_activations(scales_range)
            )
            if constant_scale is not None and scales_range is not None:
                constant_scale = min(
                    max(constant_scale, scales_range[0]), scales_range[1]
                )

            if constant_scale is not None:
                scales = inverse_scale_transform(
                    torch.tensor([constant_scale], dtype=torch.float).repeat(
                        positions.shape[0], 3
                    )
                )
        ...
        if isinstance(idx, slice) or (
            ...
            new_model = GaussianModel(
                ...
                sh_coefficients=torch.cat(
                    [new_sh_coefficients_0, new_sh_coefficients_rest], dim=1
                ),
    ...
    def concatenate(models: List["GaussianModel"]):
        ...
        concatenated_sh_coefficients_0 = torch.cat(
            [model.sh_coefficients_0 for model in models], dim=0
        )
        concatenated_sh_coefficients_rest = torch.cat(
            [model.sh_coefficients_rest for model in models], dim=0
        )
        ...
        new_model = GaussianModel(
            ...
            sh_coefficients=torch.cat(
                [concatenated_sh_coefficients_0, concatenated_sh_coefficients_rest],
                dim=1,
            ),
    ...
    def append(self, models: List["GaussianModel"]):
        ...
        if not all(isinstance(model, GaussianModel) for model in models):
            raise ValueError(
                f"All items to concatenate must be instances of GaussianModel. Got: {[type(model) for model in models]}"
            )
        ...
        # Concatenate all parameters
        concatenated_positions = torch.nn.Parameter(
            torch.cat([model.positions for model in models], dim=0)
        )
        concatenated_sh_coefficients_0 = torch.nn.Parameter(
            torch.cat([model.sh_coefficients_0 for model in models], dim=0)
        )
        concatenated_sh_coefficients_rest = torch.nn.Parameter(
            torch.cat([model.sh_coefficients_rest for model in models], dim=0)
        )
        concatenated_rotations = torch.nn.Parameter(
            torch.cat([model.rotations for model in models], dim=0)
        )
        concatenated_scales = torch.nn.Parameter(
            torch.cat([model.scales for model in models], dim=0)
        )
        concatenated_opacities = torch.nn.Parameter(
            torch.cat([model.opacities for model in models], dim=0)
        )
    ...
    def save_ply(self, filename: str):
        ...
        if (
            self.scales_range is not None
        ):  # Reparameterize to use exponential activation
        ...
        sh_coefficients_rest = (
            ...
            .cpu()
            .reshape(self.sh_coefficients_rest.shape[0], -1)
            .numpy()
        )
        ...
        sh_coefficients_0_attribs = [
            f"f_dc_{i}"
            for i in range(
                self.sh_coefficients_0.shape[1] * self.sh_coefficients_0.shape[2]
            )
        ]  # degrees * 3 channels
        sh_coefficients_rest_attribs = [
            f"f_rest_{i}"
            for i in range(
                self.sh_coefficients_rest.shape[1] * self.sh_coefficients_rest.shape[2]
            )
        ]
        ...
        dtype = [
            (attribute, "f4") for attribute in attribs
        ]  # Save all as float32 (4 bytes)
    ...
    def from_ply(filename: str, sh_channels: int = 3, device: str = "cpu"):
        ...
        sh_coefficients_0 = np.stack(sh_coefficients_0, axis=1).reshape(
            -1, 1, sh_channels
        )

        # Extract spherical harmonics coefficients for the rest
        sh_coefficients_rest = []
        extra_sh_names = sorted(
            [
                p.name
                for p in plydata["vertex"].properties
        ...
        scales = np.stack(
            [np.asarray(plydata["vertex"][name]) for name in scale_names], axis=1
        )

        rot_names = sorted(
            [p.name for p in plydata["vertex"].properties if p.name.startswith("rot")],
            key=lambda x: int(x.split("_")[-1]),
        )
        rotations = np.stack(
            [np.asarray(plydata["vertex"][name]) for name in rot_names], axis=1
        )
    ...
    def assert_validity(self):
        ...
        assert (
            ...
            == self.sh_coefficients_0.shape[0]
            == self.sh_coefficients_rest.shape[0]
            == self.rotations.shape[0]
            == self.scales.shape[0]
            == self.opacities.shape[0]
        )
