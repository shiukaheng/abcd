...
def inverse_sigmoid(x):
    return torch.log(
        x / (1 - x)
    )  # Note: If x=0, this will return -inf. If x=1, this will return nan.


def create_scaled_sigmoid(
    min: float, max: float, safety_factor=0.001
) -> Tuple[
    ...
    return (
        ...
        lambda x: inverse_sigmoid(
            torch.clamp((x - min) / (max - min), safety_factor, 1 - safety_factor)
        ),
    )
