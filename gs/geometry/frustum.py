...
class Frustum:
    ...
    """
    def __init__(self, left: Plane, right: Plane, top: Plane, bottom: Plane, near: Plane, far: Plane):
        self.left = left
        self.right = right
        self.top = top
        self.bottom = bottom
    ...
    def contains_point(self, point: torch.Tensor) -> bool:
        ...
        """
        return (self.left.signed_distance(point) <= 0 and
                self.right.signed_distance(point) <= 0 and
                self.top.signed_distance(point) <= 0 and
                self.bottom.signed_distance(point) <= 0 and
                self.near.signed_distance(point) <= 0 and
                self.far.signed_distance(point) <= 0)
    
    def intersects_bounding_box(self, box: BoundingBox) -> str:
        intersecting = True  # Start assuming the box is completely inside
        planes = [self.left, self.right, self.top, self.bottom, self.near, self.far]

        for plane in planes:
            p_vertex, n_vertex = box.get_opposing_vertices(plane.normal)
            near = plane.signed_distance(p_vertex) > 0
            far = plane.signed_distance(n_vertex) > 0

            if not near and not far:
                intersecting = False  # This means the box is completely outside the plane
                break
            elif near != far:
                intersecting = True # This means the box is intersecting the plane
    ...
    def __str__(self) -> str:
        return (f"Frustum(left={self.left}, right={self.right}, top={self.top}, " +
