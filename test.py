...
class PositionalEncoding1D(nn.Module):
    ...
    def forward(self, tensor):
        ...
        if len(tensor.shape) != 3:
            raise RuntimeError(f"The input tensor has to be 3d, but got {tensor.shape}")
...
if __name__ == "__main__":
    # I want to encode N x 3D points with 50 sinusoidal features
    N = 5
    ch = 50

    points = torch.rand(N, 3)
    # PositionaEncoding3D expects: (batchsize, x, y, z, ch)
    points = points.unsqueeze(1).unsqueeze(1).unsqueeze(1)
    print(points.shape, points)

    penc = PositionalEncoding3D(ch)

    out = penc(points)
    print(out.shape, out)

    print("Parameters", list(penc.parameters()))
